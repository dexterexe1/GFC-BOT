...
Supports a dedicated cluster via MONGO_URI_REVENUE (falls back to MONGO_URI).
Use a separate free Atlas cluster here to get another 512 MB for revenue only.
"""
import os
import datetime
...
# Prefer dedicated revenue cluster; else shared MONGO_URI
MONGO_URI = (
    os.getenv("MONGO_URI_REVENUE")
    or os.getenv("MONGO_URI")
    or os.getenv("MONGODB_URL")
)
MONGO_DB = os.getenv("MONGO_DB_REVENUE") or os.getenv("MONGO_DB", "bunnydb_revenue")
...
async def purge_old_revenue_entries(days: int = 90) -> int:
    """Delete revenue entries older than `days` (default 90 = 3 months). All guilds.

    Keeps revenue_channels / revenue_managers config. Returns deleted count.
    """
    if db is None:
        return 0
    try:
        cutoff = datetime.datetime.now(UTC) - datetime.timedelta(days=int(days))
        result = await db["revenue_entries"].delete_many({"timestamp": {"$lt": cutoff}})
        n = int(result.deleted_count)
        if n:
            print(f"🧹 Revenue auto-clean: deleted {n} entries older than {days} days (before {cutoff.isoformat()})")
        else:
            print(f"🧹 Revenue auto-clean: nothing older than {days} days")
        return n
    except Exception as e:
        print(f"⚠️ Failed to purge old revenue entries: {e}")
        return 0
