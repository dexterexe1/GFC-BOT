...
async def _execute_action(guild: discord.Guild, plan: Dict[str, Any]) -> str:
    ...
    if action == "create_role":
        ...
        try:
            ...
            # Optional placement requested by the AI, e.g. "just below Administration".
            # Discord role positions are higher when the role appears higher in the list.
            below_name = str(plan.get("below_role") or "").strip()
            if below_name:
                below_role = discord.utils.find(
                    lambda r: r.name.casefold() == below_name.casefold(), guild.roles
                )
                if below_role and below_role != role and below_role < bot_top and role < bot_top:
                    target_position = max(1, below_role.position - 1)
                    await guild.edit_role_positions(
                        positions={role: target_position},
                        reason=f"AI Manager placement requested by {plan.get('requested_by')}",
                    )
            ...
            # Optional immediate assignment when the request says to create AND give the role.
            target_id = int(plan.get("user_id") or 0)
            if target_id:
                member = guild.get_member(target_id)
                if member and role < bot_top:
                    await member.add_roles(
                        role,
                        reason=f"AI Manager requested by {plan.get('requested_by')}",
                    )
                    return f"✅ Created role **{role.name}**, placed it as requested, and assigned it to **{member.display_name}**."
        except Exception:
...
- none
- create_role: {{name, permissions[], color?, below_role?, user_id?}}
...
- If the user asks to create a role "below", "under", "just below", or "a bit lower than" a named role, set below_role to that exact role name.
- If the user asks to create a role and give/assign it to a mentioned user in the same request, set user_id to that mentioned ID.
- If the user says "staff" but gives no exact role name, use "Staff" as the role name.
- Staff/moderator roles must never receive Administrator or other privileged permissions.
...
def _looks_like_action_request(question: str) -> bool:
    ...
    starters = (
        ...
        "create staff role", "make staff role", "create a staff role", "make a staff role",
        "create moderator role", "make moderator role", "create a moderator role", "make a moderator role",
        "create channel", "make channel", "create a channel", "make a channel",
        "create category", "make category", "create a category", "make a category",
        "rename role", "rename the role", "give role", "give the role", "add role", "add the role",
        "remove role", "remove the role", "take role", "take the role",
    )
