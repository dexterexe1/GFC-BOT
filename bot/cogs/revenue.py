...
from bot.revenue_database import (
    ...
    get_revenue_managers_due, mark_revenue_manager_weekly_dm, purge_old_revenue_entries
)
...
# --- AUTO-CLEAN: drop revenue entries older than 3 months ---
_revenue_cleanup_task = None
REVENUE_RETENTION_DAYS = 90  # 3 months


async def revenue_cleanup_loop():
    """Once per day, delete revenue rows older than 3 months."""
    await asyncio.sleep(30)  # let DB connect after boot
    while True:
        try:
            from bot.revenue_database import db as _rev_db
            if _rev_db is not None:
                await purge_old_revenue_entries(REVENUE_RETENTION_DAYS)
            else:
                # try ensure init already ran
                await purge_old_revenue_entries(REVENUE_RETENTION_DAYS)
        except Exception as exc:
            print(f"⚠️ Revenue cleanup loop error: {exc}")
        await asyncio.sleep(24 * 60 * 60)  # daily


def start_revenue_cleanup_loop():
    global _revenue_cleanup_task
    if _revenue_cleanup_task is None or _revenue_cleanup_task.done():
        _revenue_cleanup_task = asyncio.create_task(revenue_cleanup_loop())
        print("✅ Revenue 3-month auto-clean loop started")
...
def start_revenue_manager_weekly_loop():
    ...
    try:
        start_revenue_cleanup_loop()
    except Exception as _e:
        print(f"⚠️ Revenue cleanup loop not started: {_e}")
