...
async def on_ready():
    ...
    # Private lock: leave any guild not on the allow-list
    if PRIVATE_GUILD_LOCK and ALLOWED_GUILD_IDS:
        for g in list(bot.guilds):
            if not guild_is_allowed(g.id):
                print(f"🔒 Leaving non-allowed guild on ready: {g.name} ({g.id})")
                try:
                    await g.leave()
                except Exception as e:
                    print(f"⚠️ Failed to leave guild {g.id}: {e}")
    ...
    if TICKETS_ENABLED and TicketPanelView:
        bot.add_view(TicketPanelView())
    if TICKETS_ENABLED and TicketManageView:
        bot.add_view(TicketManageView())
    # Applications removed — skip ControlPanelView registration
    # if ControlPanelView:
...
async def on_guild_join(guild):
    ...
    if PRIVATE_GUILD_LOCK and not guild_is_allowed(guild.id):
        print(f"🔒 Leaving non-allowed guild: {guild.name} ({guild.id})")
        try:
            await guild.leave()
        except Exception as e:
            print(f"⚠️ Failed to leave guild: {e}")
        return
    await publish_bot_status()
