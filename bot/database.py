...
# Always next to project root (same folder as run_bot.py), not dependent on cwd
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DB_FILE = os.getenv("BOT_DB_PATH") or os.path.join(_ROOT, "bot_data.db")
...
# Vouch + shared mod cluster (same as AI / mongo_bridge)
_VOUCH_MONGO_URI = os.getenv("MONGO_URI_MOD") or os.getenv("MONGO_URI") or os.getenv("MONGODB_URL")
_VOUCH_MONGO_DB = os.getenv("MONGO_DB_MOD") or os.getenv("MONGO_DB", "bunnydb")
_vouch_client = None
_vouch_db = None


def _vouch_mongo():
    """Lazy connect to same Mongo as AI/moderation. Returns db or None."""
    global _vouch_client, _vouch_db
    if not _VOUCH_MONGO_URI or MongoClient is None:
        return None
    if _vouch_db is not None:
        return _vouch_db
    try:
        _vouch_client = MongoClient(_VOUCH_MONGO_URI, serverSelectionTimeoutMS=8000)
        _vouch_client.admin.command("ping")
        _vouch_db = _vouch_client[_VOUCH_MONGO_DB]
        # indexes once
        _vouch_db["vouches"].create_index([("guild_id", 1), ("target_id", 1)])
        _vouch_db["vouches"].create_index([("guild_id", 1), ("target_id", 1), ("author_id", 1)])
        _vouch_db["vouch_channels"].create_index("guild_id", unique=True)
        print(f"✅ Vouch Mongo connected ({_VOUCH_MONGO_DB})")
        return _vouch_db
    except Exception as e:
        print(f"⚠️ Vouch Mongo unavailable, using SQLite fallback: {e}")
        return None
...
def init_db():
    ...
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_warn_logs_user ON warn_logs(guild_id, user_id)")
    cursor.execute("""
...
def add_warn_log(guild_id: int, user_id: int, moderator_id: int, reason: str):
    """Record a single warning with reason (unlimited history)."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO warn_logs (guild_id, user_id, moderator_id, reason) VALUES (?, ?, ?, ?)",
        (guild_id, user_id, moderator_id, reason or "No reason provided"),
    )
    conn.commit()
    conn.close()


def get_warn_logs(guild_id: int, user_id: int, limit: int = 5):
    """Return recent warn logs: list of (moderator_id, reason, created_at)."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
        """
...
def clear_warn_logs(guild_id: int, user_id: int):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM warn_logs WHERE guild_id = ? AND user_id = ?", (guild_id, user_id))
    conn.commit()
    conn.close()
...
def add_vouch(guild_id: int, target_id: int, author_id: int, comment: str = None):
    """Store vouch on same Mongo as AI/moderation when available."""
    db = _vouch_mongo()
    created = datetime.datetime.now(UTC).isoformat()
    if db is not None:
        db["vouches"].insert_one({
    ...
    cursor.execute(
        ...
        (guild_id, target_id, author_id, comment, created),
    )
    conn.commit()
    conn.close()
...
def remove_last_vouch(guild_id: int, target_id: int, author_id: int) -> bool:
    """Remove the most recent vouch the given author gave to target."""
    db = _vouch_mongo()
    if db is not None:
        doc = db["vouches"].find_one(
            {"guild_id": int(guild_id), "target_id": int(target_id), "author_id": int(author_id)},
            sort=[("_id", DESCENDING)],
        )
        if not doc:
            return False
        db["vouches"].delete_one({"_id": doc["_id"]})
        return True
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
...
def staff_remove_vouch(guild_id: int, target_id: int) -> bool:
    """Remove the most recent vouch for target regardless of who gave it."""
    db = _vouch_mongo()
    if db is not None:
        doc = db["vouches"].find_one(
            {"guild_id": int(guild_id), "target_id": int(target_id)},
            sort=[("_id", DESCENDING)],
        )
        if not doc:
            return False
...
def staff_clear_all_vouches(guild_id: int, target_id: int) -> int:
    """Delete every vouch for target in this guild. Returns count removed."""
    db = _vouch_mongo()
    if db is not None:
        result = db["vouches"].delete_many(
            {"guild_id": int(guild_id), "target_id": int(target_id)}
        )
...
def count_vouches(guild_id: int, target_id: int) -> int:
    db = _vouch_mongo()
    if db is not None:
        return int(db["vouches"].count_documents(
            {"guild_id": int(guild_id), "target_id": int(target_id)}
        ))
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
...
def list_vouches(guild_id: int, target_id: int, limit: int = 5):
    db = _vouch_mongo()
    if db is not None:
        cursor = db["vouches"].find(
            {"guild_id": int(guild_id), "target_id": int(target_id)},
            sort=[("_id", DESCENDING)],
        ).limit(int(limit))
        return [(d.get("author_id"), d.get("comment"), d.get("created_at")) for d in cursor]
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
...
def vouch_leaderboard(guild_id: int, limit: int = 10):
    ...
    if db is not None:
        pipeline = [
            ...
            {"$group": {"_id": "$target_id", "c": {"$sum": 1}}},
            {"$sort": {"c": -1}},
            {"$limit": int(limit)},
        ]
        return [(d["_id"], d["c"]) for d in db["vouches"].aggregate(pipeline)]
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute(
...
def get_vouch_channel(guild_id: int):
    db = _vouch_mongo()
    if db is not None:
        doc = db["vouch_channels"].find_one({"guild_id": int(guild_id)})
...
def clear_vouch_channel(guild_id: int):
    db = _vouch_mongo()
    if db is not None:
        db["vouch_channels"].delete_one({"guild_id": int(guild_id)})
        return
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
